...
class AddTransaction extends Component {
    ...
    handleSubmit = (event) => {
        ...
        if (this.state.type === '2') {
            balance = parseInt(this.state.currentBalance) - parseInt(this.state.amount);
            if (balance < 0) {
                alert('Stan konta nie może być ujemny!')
            }
            else {
                { update() }
            }
                        ...
                        <div className="col-md-8 m-auto">
                            ...
                            <form onSubmit={this.handleSubmit}>
                                <div style={{
                                    ...
                                    <input type="number" min="1" value={amount} onChange={event => this.changeHandler(event, "amount", false)} className={classnames("form-control form-control-lg", { "is-invalid": this.state.errors.amount })} placeholder="Kwota" required />
